/**
 * TimeManager
 * Game Logic layer - manages game time progression and scheduling
 */
...
export interface TimeEvent {
  ...
  recurring?: boolean;
  dayOfWeek?: DayOfWeek; // For weekly recurring events
  fired?: boolean;       // Track if one-time event has fired
}

export class TimeManager {
  private hour: number = 9;           // Start at 9:00 AM
  private minute: number = 0;
  private dayOfWeek: DayOfWeek = 'Monday';
  private dayNumber: number = 1;

  private timeScale: number = 1;      // 1 = normal speed, 0 = paused
  private accumulatedTime: number = 0; // Milliseconds

  private events: Map<string, TimeEvent> = new Map();
  private nextEventId: number = 1;

  // How many real milliseconds = 1 game minute
  // 1000ms = 1 game minute means 1 real second = 1 game minute (60x speed)
  // 60000ms = 1 game minute means real-time (1:1)
   ...
   * Call this every frame with deltaMs from Phaser
   */
  update(deltaMs: number): void {
    ...
    // Convert accumulated time to game minutes
  ...
  private checkEvents(): void {
    ...
    this.events.forEach(event => {
      // Skip if already fired (one-time events)
      if (!event.recurring && event.fired) return;

      // Check if day of week matches (for weekly recurring)
  ...
  setTime(hour: number, minute: number): void {
    ...
    // If day changed, trigger events for the new time
  ...
  exportState(): any {
    const events: any[] = [];
    this.events.forEach(event => {
      // Don't save callbacks, they need to be re-registered
      events.push({
        id: event.id,
        hour: event.hour,
        minute: event.minute,
    ...
    return {
      ...
      events, // Event metadata only
    };
  }

  /**
   * Import state from save data
   * Note: Event callbacks need to be re-registered after import
   */
  importState(data: any): void {
    ...
    // Note: Events are not restored here as callbacks can't be serialized
    // Game code needs to re-register events after loading
    this.events.clear();
  }
}
